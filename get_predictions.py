...
def main():
    ...
    for cv_set_name in cv_set_dirs:
        ...
        hyperparams_name = 'SG_{}_M{}x{}_T{}_c{}.p{}.t{}.resunit{}.lr{}'.format(
            cv_set_name, map_width, map_height, len_timeslot, len_closeness, len_period, len_trend, nb_residual_unit, lr
        )
        fname_param = os.path.join('MODEL', '{}_cont.h5'.format(hyperparams_name))
        ...
        print(Y_test.shape)
        np.save('PREDICTIONS/' + cv_set_name + '/stresnet_predictions.npy', predictions)
        np.save('PREDICTIONS/' + cv_set_name + '/true_y.npy', Y_test)
        np.save('PREDICTIONS/' + cv_set_name + '/timestamps.npy', timestamp_test)
