...
log_fname = time.strftime('%Y-%m-%d_%H-%M-%S_', local_time) + baseline_model
log_fname += '.out'
...
def main():
    ...
    for i, test_t in enumerate(timestamp_test):
        # logging.info('predicting for %d/%d' % (i + 1, len(timestamp_test)))
        ...
        weekday = datetime.strptime(date, '%Y%m%d').weekday()
        predictions.append(np.array(hist[weekday][timeslot]))
    predictions = np.array(predictions)
    predictions_fname = 'histavg_predictions.npy'
